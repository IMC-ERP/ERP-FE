...
import { 
  Store, User, MapPin, Phone, Calendar, Upload, 
...
export const SettingsPage = () => {
  ...
  const handleProfileChange = (field: keyof typeof storeProfile, value: string) => {
        ...
        <div className="w-full md:w-64 flex-shrink-0">
          <nav className="flex flex-row md:flex-col bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
            <button
              ...
              className={`flex-1 md:flex-none flex items-center gap-3 px-4 py-3 text-sm font-medium transition-colors ${
                activeTab === 'profile' ? 'bg-blue-50 text-blue-700 border-l-4 border-blue-600' : 'text-slate-600 hover:bg-slate-50'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 md:flex-none flex items-center gap-3 px-4 py-3 text-sm font-medium transition-colors ${
                activeTab === 'display' ? 'bg-blue-50 text-blue-700 border-l-4 border-blue-600' : 'text-slate-600 hover:bg-slate-50'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 md:flex-none flex items-center gap-3 px-4 py-3 text-sm font-medium transition-colors ${
                activeTab === 'notification' ? 'bg-blue-50 text-blue-700 border-l-4 border-blue-600' : 'text-slate-600 hover:bg-slate-50'
              }`}
            >
              <Bell size={18} /> 알림 설정
            </button>
                ...
                <div>
                   ...
                   <div className="flex flex-wrap gap-4">
                      {themeColors.map(color => (
                         <button
                           key={color.id}
                           onClick={() => updateAppSettings({ themeColor: color.id as any })}
                           className={`group relative flex items-center gap-2 p-2 pr-4 rounded-full border transition-all ${
                   ...
                   <div className="flex gap-4">
                      <button 
                        ...
                        className={`flex-1 p-4 rounded-xl border flex flex-col items-center gap-3 transition-all ${
                           !appSettings.darkMode ? 'border-blue-500 bg-blue-50 text-blue-700 ring-1 ring-blue-500' : 'border-slate-200 hover:bg-slate-50'
                        }`}
                      >
                         <Sun size={24} />
                      ...
                      <button 
                        ...
                        className={`flex-1 p-4 rounded-xl border flex flex-col items-center gap-3 transition-all ${
                           appSettings.darkMode ? 'border-slate-800 bg-slate-800 text-white' : 'border-slate-200 hover:bg-slate-50'
                        }`}
                      >
                         <Moon size={24} />
                      ...
                      <div key={item.id} className="flex items-center justify-between p-4 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors">
                         ...
                         <button 
                           ...
                           className={`w-12 h-6 rounded-full transition-colors relative ${
                              appSettings.notifications[item.id as keyof typeof appSettings.notifications] 
                                 ? 'bg-blue-600' 
                                 : 'bg-slate-300'
                           }`}
                         >
