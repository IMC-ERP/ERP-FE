...
export default function SettingsPage() {
    ...
    // ========== 계정 정보 상태 ==========
    const [accountData, setAccountData] = useState<import('../services/api').UserProfile>({
        uid: '', email: '', phone: '', store_name: '', role: '', store_id: '', name: '', created_at: '', updated_at: ''
    });
    ...
    useEffect(() => {
        const loadAll = async () => {
            ...
            try {
                ...
                setStoreProfile(res.data as unknown as StoreProfile);
                setTempProfile({
    ...
    const saveProfile = async () => {
        try {
            ...
            if (tempProfile.owner_name !== storeProfile?.owner_name) updateData.owner_name = tempProfile.owner_name;
            if (tempProfile.contact_number !== storeProfile?.contact_number) updateData.contact_number = tempProfile.contact_number;
            if (tempProfile.address !== storeProfile?.address) updateData.address = tempProfile.address;
            const yearValue = tempProfile.established_year;
            const parsedYear = typeof yearValue === 'string' ? (yearValue ? parseInt(yearValue) : null) : yearValue;
            if (parsedYear !== storeProfile?.established_year && parsedYear !== null) updateData.established_year = parsedYear;
    ...
    const saveAccountInfo = async () => {
        try {
            ...
            const res = await userApi.updateProfile(patchData);
            setAccountData(res.data);
            setAccountSaved(true);
            setTimeout(() => setAccountSaved(false), 2000);
        } catch (err: any) {
                                    ...
                                    <>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            ...
                                            <div>
                                                ...
                                                <input type="text" value={tempProfile.owner_name} disabled={!isOwner} onChange={e => handleProfileChange('owner_name', e.target.value)} className="w-full p-3 border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-blue-500 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200" />
                                            </div>
                                            <div>
                                                <label className="block text-sm font-medium text-slate-700 mb-2 flex items-center gap-1"><Calendar size={14} /> 설립연도</label>
                                                <input type="number" value={tempProfile.established_year} disabled={!isOwner} onChange={e => handleProfileChange('established_year', e.target.value)} placeholder="예: 2024" min="1900" max="2100" className="w-full p-3 border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-blue-500 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200" />
                                            </div>
                                            <div>
                                                <label className="block text-sm font-medium text-slate-700 mb-2 flex items-center gap-1"><Phone size={14} /> 연락처</label>
                                                <input type="text" value={tempProfile.contact_number} disabled={!isOwner} onChange={e => handleProfileChange('contact_number', e.target.value)} className="w-full p-3 border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-blue-500 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200" />
                                            </div>
