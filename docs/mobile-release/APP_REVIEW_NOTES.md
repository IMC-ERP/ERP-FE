...
- 로그인 방식: Apple 또는 Google 계정 기반 로그인
- 테스트 계정 이메일: `<reviewer-account@example.com>`
- 테스트 계정 비밀번호/접근 방법: `<필요 시 입력>`
- 테스트 매장명: `<review store>`
- iOS 리뷰 권장 경로: `Sign in with Apple` 또는 제공된 테스트 Google 계정 사용

## 리뷰어가 확인할 핵심 경로

1. 로그인 화면에서 Apple 또는 Google 로그인 진행
...
- 앱은 Login Services 가이드 대응을 위해 Apple 로그인을 함께 제공합니다.
